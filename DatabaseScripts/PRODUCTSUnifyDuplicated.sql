...
(SELECT SUBSTRING(
		...
		= 'AreaId,CfdBaseAreaId,CommodityId,DeliveryTypeId,ExecutionVenueId,FirstTradeDate,FromDate,FutureTypeId,Hours,IsCall,LastTradeDate,LoadTypeId,PriceUnitId,ProdId,ProdName,ProductVenueId,PublishingPeriodTypeId,SourceId,Strike,TimeStamp,ToDate,TypeId')
begin
	THROW 50001, 'The columns considered by this script are different than the columns in Products table. Please contact a developer to analyse the differences', 1
end
...
(SELECT SUBSTRING(
        (
			...
			FROM        sys.columns c
			JOIN        sys.tables  t   ON c.object_id = t.object_id
			WHERE       t.name = 'ProductCodes'
			ORDER BY    c.name
            FOR XML PATH ('')
        ), 2, 1000))
		= 'Code,ProductCodeTypeId,ProductId')
begin
	THROW 50001, 'The columns considered by this script are different than the columns in the ProductCodes table. Please contact a developer to analyse the differences', 1
end
...
(SELECT SUBSTRING(
		...
		= 'CurrencyId,ProdId,SourceId,TimeStamp,TradeDate,TypeId,Value')
begin
	THROW 50001, 'The columns considered by this script are different than the columns in the Prices table. Please contact a developer to analyse the differences', 1
end
