IF NOT EXISTS ( SELECT 1 FROM [IberiaECM].[dbo].InstallShield WHERE ISSchema = '2017.1.1.06' )
BEGIN
	THROW 50001, 'The script cannot run in ECM versions prior to 2017.1.1.06', 1
END

if ('2018.2.1.01' < (SELECT MAX(ISSchema) FROM [IberiaPrices].[dbo].InstallShield) OR '2019.1.0.03' < (SELECT MAX(ISSchema) FROM [IberiaECM].[dbo].InstallShield))
BEGIN
...
if not exists (select * from [IberiaECM].[dbo].sysobjects where name='ELVIZ14213PublishingPeriodTypeIdBkp' and xtype='U')
	CREATE TABLE [IberiaECM].[dbo].[ELVIZ14213PublishingPeriodTypeIdBkp](
...
BEGIN TRY
		...
		UPDATE [IberiaECM].[dbo].Instruments
		...
		INTO [IberiaECM].[dbo].ELVIZ14213PublishingPeriodTypeIdBkp
		FROM 
		(
			select InstrumentId, MIN(patterns.PeriodTypeId) PeriodTypeId
			FROM [IberiaECM].[dbo].Instruments i
			JOIN
			(
				...
				DisplayTickerRule, '{Ww}', '[0-9][0-9]'),
					...
					'{y}', '[0-9]') pattern
				from [IberiaECM].[dbo].FutureConfiguration fc
				JOIN [IberiaECM].[dbo].PeriodTypes pt
		...
		JOIN [IberiaECM].[dbo].Instruments i
