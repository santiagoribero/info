...
if not exists (select * from [P26Prices].[dbo].sysobjects where name='ELVIZ14213PublishingPeriodTypeIdBkp' and xtype='U')
	CREATE TABLE [P26Prices].[dbo].[ELVIZ14213PublishingPeriodTypeIdBkp](
		[ProdId] [int] NOT NULL,
...
BEGIN TRY
		...
		INTO [P26Prices].[dbo].ELVIZ14213PublishingPeriodTypeIdBkp
		FROM 
		(
			...
			(
				...
				JOIN [P26Prices].[dbo].PeriodTypes pt
				on fc.Period = pt.PeriodTypeName
				JOIN [P26ECM].[dbo].Commodities c
				ON fc.CommodityId = c.CommodityId
			) patterns
			on (p.ProdName like patterns.pattern and p.CommodityId = patterns.VizPricesCommodityId)
			where p.TypeId = 3 --UPDATE ONLY Future [P26Prices].[dbo].Products
			and p.PublishingPeriodTypeId IS NULL --UPDATE ONLY [P26Prices].[dbo].Products that don't have a PublishingPeriodTypeId
			group by ProdId
			having count(*) = 1 --UPDATE ONLY if it is clear which one is the PeriodTypeId for the Instrument


		) newPublishingPeriodTypeId
		JOIN [P26Prices].[dbo].Products p
