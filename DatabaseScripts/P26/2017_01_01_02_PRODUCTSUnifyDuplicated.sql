IF NOT EXISTS ( SELECT 1 FROM [P26Prices].[dbo].InstallShield WHERE ISSchema = '2017.1.1.02' )
BEGIN
...
if not exists (select * from [P26Prices].[dbo].sysobjects where name='ELVIZ14214ProductsBkp' and xtype='U')
CREATE TABLE [P26Prices].[dbo].[ELVIZ14214ProductsBkp](
	[ProdId] [int] NOT NULL,
...
if not exists (select * from [P26Prices].[dbo].sysobjects where name='ELVIZ14214PricesBkp' and xtype='U')
CREATE TABLE [P26Prices].[dbo].[ELVIZ14214PricesBkp](
...
BEGIN TRY
		...
		(
			...
			FROM [P26Prices].[dbo].Products p
			LEFT JOIN [P26Prices].[dbo].ProductCodes pc
			ON p.ProdId = pc.ProductId
			JOIN [P26Prices].[dbo].DataSources ds
			...
			JOIN (
					...
					FROM [P26Prices].[dbo].Products p2
					LEFT JOIN [P26Prices].[dbo].ProductCodes pc2
					ON p2.ProdId = pc2.ProductId
					JOIN [P26Prices].[dbo].DataSources ds2
		...
		INSERT INTO [P26Prices].[dbo].Prices ([ProdId],[TradeDate],[TypeId],[Value],[SourceId],[TimeStamp],[CurrencyId])
		SELECT subquery.FirstProdId, p3.[TradeDate], p3.[TypeId], p3.[Value], p3.[SourceId], p3.[TimeStamp], p3.[CurrencyId]
		FROM [P26Prices].[dbo].Prices p3
		JOIN
		(
