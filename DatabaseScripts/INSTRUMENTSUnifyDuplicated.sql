...
:setvar ECMdbname "DAILY_QAECM193"
...
(SELECT SUBSTRING(
		...
		= 'Amount,AuctionTypeId,AuthConfirmed,AuthId,BalanceAreaId,BFeeCurrencyId,CapacityBidPrice,CapacityBidVolume,CapacityId,CapacityTradeVolume,CashSettlementPeriodContainerId,CertificateLimitFactor,CFeeCurrencyId,ClearingTypeId,Comment,ConfirmedByBroker,ConfirmedByCounterparty,ContractForm,ContractSplitId,ContractType,CounterpartyTrader,CtCparty,CurrencyId,CurrencyMargin,CurrencySourceId,CustomCompanyId,CustomCompanyName,DealCompression,DeclaredVolume,DeclareId,DeliveryTypeId,DestinationReferencePriceSeriesId,DistrAmount,DistrParentId,EMIRStatus,EnvironmentLabelId,ExchangedFrom,ExchangeId,ExchangeRate,ExpiryTime,ExternalId,ExternalSourceName,FixPrice,FlexibleTimeSeries,FromCountryId,GridPointId,GroupField1,GroupField2,GroupField3,HistoricContractPriceSeriesId,HistoricMarketPriceSeriesId,Id,IndexedPriceBookTemplateId,InitialPrice,InitiatorAggressorId,InstrumentId,InterconnectorId,Interruptible,LastUpdatedUTC,LoadProfileId,LocationId,LossFactor,LowerTrigger,MarketPriceMultiplicator,Markup,MasterAgreementId,MaxVol,MinVol,ModelTypeId,NotedTime,OrderId,OrgAmount,OrgPrice,OrgPriceUnitId,OrgVolumeUnitId,Originator,OTCBroker,Paid,Payout,PortfolioId,Price,PriceBasisId,PricebasisToAreaId,PriceUnitId,ReferencePriceSeriesId,ReferringId,RemitReporting,ReplicationId,RiskValue,SamplingFrom,SamplingTo,SchedulingAccountTypeId,SchedulingNominationProcedureId,SettlementRuleId,StatusId,SwapPriceTypeId,T_BDA_Id,TerminationDate,Threshold,TicketNumber,TimeStampClearedUTC,TimeStampConfirmationBrokerUTC,TimeStampConfirmationCounterpartyUTC,TimeZoneId,TimeZoneOffset,ToCountryId,TradeCompanyId,TradeDate,TraderId,TradeTime,TransTypeId,TriggerCurrencyId,TU_Id,TUP_Id,UnderlyingDeliveryTypeId,UnderlyingInstrumentID,UpperTrigger,UserId,UserSpecifiedName,UTI,VT_ID')
begin
	THROW 50001, 'The columns considered by this script are different than the columns in Transactions table. Please contact a developer to analyse the differences', 1
end
...
(SELECT SUBSTRING(
        (
			...
			FROM        sys.columns c
			JOIN        sys.tables  t   ON c.object_id = t.object_id
			WHERE       t.name = 'Instruments'
			ORDER BY    c.name
            FOR XML PATH ('')
        ), 2, 1000))
		= 'BarrierOptionTypeId,BaseCurrencyId,BookPriceTypeId,CapFloorPricingPeriodTypeId,CommodityId,CrossCurrencyId,CurrencyId,CurrencySourceId,DeliveryTypeId,DestinationReferencePriceSeriesId,EnvironmentLabelId,ExecutionVenueId,ExpiryDate,ExpiryOffset,ExpiryTime,FixPrice,FromDate,FutureTypeId,HistoricContractPriceSeriesId,HistoricMarketPriceSeriesId,Hours,IndexedPriceBookTemplateId,InstrumentId,InstrumentName,InstrumentShortName,InstrumentTypeId,InterconnectorId,Interruptible,LoadProfileId,LowerTrigger,MarketPriceFactor,MaxVol,MinVol,ModelTypeId,Payout,PriceBasisId,PriceBasisToAreaId,PriceUnitId,PublishingPeriodTypeId,PutCall,ReferencePriceSeriesId,SamplingFrom,SamplingPeriodId,SamplingTo,SettlementRuleId,StatusId,Strike,StripPeriodResolution,SwapPriceTypeId,Threshold,TimeZoneId,ToDate,TriggerCurrencyId,TU_Id,UnderlyingInstrumentId,UpperTrigger,UserSpecifiedName')
begin
	THROW 50001, 'The columns considered by this script are different than the columns in the Instruments table. Please contact a developer to analyse the differences', 1
end
...
(SELECT SUBSTRING(
        (
			...
			FROM        sys.columns c
			JOIN        sys.tables  t   ON c.object_id = t.object_id
			WHERE       t.name = 'InstrumentCertificates'
			ORDER BY    c.name
            FOR XML PATH ('')
        ), 2, 1000))
		= 'CertificateTypeId,DateOfTransfer,InstrumentId,ProductionFacilityId')
begin
	THROW 50001, 'The columns considered by this script are different than the columns in the InstrumentCertificates table. Please contact a developer to analyse the differences', 1
end
